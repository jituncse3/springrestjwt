...
import com.nse.exception.ErrorDetails;
	...
	public Object run() {
		try {
			...
			if (ctx != null && ctx.getResponseStatusCode() == 500) {
				...
				ctx.setResponseBody(new ObjectMapper().writerWithDefaultPrettyPrinter()
						.writeValueAsString(new ErrorDetails(new Date(), "500", "Internal Server Error",
								"Something went wrong", ctx.getRequest().getRequestURI().toString()))); // Custom Error
																										// Message
