...
public class EmployeeAction extends ActionSupport implements ModelDriven<Employee> {
	...
	public String create() {
		...
		WebTarget webTarget = client.target("http://localhost:9019/abcd").path("addEmployee");
	 ...
	 * System.out.println(employee.getId()); Invocation.Builder invocationBuilder =
	 * webTarget.request(MediaType.APPLICATION_XML); Response response =
	 * invocationBuilder.get();
	 * 
	 * employee = response.readEntity(Employee.class); System.out.println("Result"+
	 * employee); System.out.println(response.getStatus());
	 * System.out.println(employee); return "success"; }
	 */
	/*
	 ...
	 * 
	 * final Client client =
	 * ClientBuilder.newBuilder().register(MultiPartFeature.class).build(); final
	 * FileDataBodyPart filePart = new FileDataBodyPart("file",
	 * employee.getFileUpload1()); System.out.println("File path"+
	 * employee.getFileUpload1()); FormDataMultiPart formDataMultiPart = new
	 * FormDataMultiPart(); final FormDataMultiPart multipart = (FormDataMultiPart)
	 * formDataMultiPart.field("foo", "bar") .bodyPart(filePart);
	 * System.out.println("testing");
	 * 
	 * final WebTarget target =
	 * client.target("http://localhost:8070/Struts2jdbc-02/rest/getemployees/pdf");
	 * final Response response = target.request().post(Entity.entity(multipart,
	 * multipart.getMediaType())); //
	 * System.out.println(response.readEntity(Employee.class));
	 * formDataMultiPart.close(); multipart.close(); return "success";
	 * 
	 * }
	 */
